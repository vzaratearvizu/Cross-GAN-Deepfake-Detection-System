...
print("Building TRAIN dataset (StyleGAN1 - Flickr)...")
X_train, y_train = build_features(TRAIN_REAL_DIR, TRAIN_FAKE_DIR, MAX_TRAIN, "(StyleGAN1 train)")
print(f"Train shape: {X_train.shape}")

print("Building TEST dataset (StyleGAN2 - SFHQ)...")
X_test, y_test = build_features(TEST_REAL_DIR, TEST_FAKE_DIR, MAX_TEST, "(StyleGAN2 test)")
...
# ── Plot ──────────────────────────────────────────────────────────────────────
folder_path = experiments_folder("dct-model-v9-experiment")
...
plt.ylabel('Accuracy')
plt.title('Cross-GAN: StyleGAN1 (Flickr) → StyleGAN2 (SFHQ)')
plt.legend()
plt.grid(True)
