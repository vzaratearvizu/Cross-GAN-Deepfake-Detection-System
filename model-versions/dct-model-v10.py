...
def experiments_folder(base_name):
    ...
    n = 1
    while os.path.exists(os.path.join(results_path, f"{base_name}-{n}")):
        n += 1
    folder_path = os.path.join(results_path, f"{base_name}-{n}")
...
# ── Load images ───────────────────────────────────────────────────────────────
def build_features(real_dir, fake_dir, max_per_class, label="", nested=False):
    def get_files(d):
        if nested:
            files = []
            ...
            return files
        return [os.path.join(d, f) for f in os.listdir(d) if f.endswith(('.jpg', '.png'))]
    ...
    print(f"Extracting real images {label}...")
    ...
    print(f"Extracting fake images {label}...")
...
print("Building TRAIN dataset (Stable Diffusion)...")
X_train, y_train = build_features(TRAIN_REAL_DIR, TRAIN_FAKE_DIR, MAX_TRAIN, "(Stable Diffusion train)", nested=True)
print(f"Train shape: {X_train.shape}")

print("Building TEST dataset (StyleGAN2 - SFHQ)...")
X_test, y_test = build_features(TEST_REAL_DIR, TEST_FAKE_DIR, MAX_TEST, "(StyleGAN2 test)", nested=False)
...
# ── Plot ──────────────────────────────────────────────────────────────────────
folder_path = experiments_folder("dct-model-v10-experiment")
...
plt.plot(train_scores, label='Train Accuracy (Stable Diffusion)')
plt.plot(test_scores, label='Test Accuracy (StyleGAN2)')
plt.xlabel('Number of Trees')
plt.ylabel('Accuracy')
plt.title('Cross-GAN: Stable Diffusion → StyleGAN2 (SFHQ)')
plt.legend()
plt.grid(True)
...
with open(os.path.join(folder_path, "results.txt"), "w") as f:
    ...
    f.write(f"Train Dataset: stable-diffusion-dataset (Stable Diffusion V1.5)\n")
    f.write(f"Test Dataset: sfhq-part3 (StyleGAN2)\n")
