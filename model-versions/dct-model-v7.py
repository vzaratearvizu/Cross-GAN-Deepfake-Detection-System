...
# ── Dirs ─────────────────────────────────────────────────────────────────────
# Train on StyleGAN2 (SFHQ)
TRAIN_REAL_DIR = os.path.join("datasets", "flickr-gan-dataset", "real_vs_fake", "real-vs-fake", "train", "real")
TRAIN_FAKE_DIR = os.path.join("datasets", "sfhq-part3", "images", "images")

# Test on StyleGAN1 (Flickr)
...
# ── Load images ───────────────────────────────────────────────────────────────
def build_features(real_dir, fake_dir, max_per_class, label=""):
...
print("Building TRAIN dataset (StyleGAN2 - SFHQ)...")
X_train, y_train = build_features(TRAIN_REAL_DIR, TRAIN_FAKE_DIR, MAX_TRAIN, "(StyleGAN2 train)")
print(f"Train shape: {X_train.shape}")

print("Building TEST dataset (StyleGAN1 - Flickr)...")
X_test, y_test = build_features(TEST_REAL_DIR, TEST_FAKE_DIR, MAX_TEST, "(StyleGAN1 test)")
...
# ── Plot ──────────────────────────────────────────────────────────────────────
folder_path = experiments_folder("dct-model-v7-experiment")
...
plt.plot(train_scores, label='Train Accuracy (StyleGAN2)')
plt.plot(test_scores, label='Test Accuracy (StyleGAN1)')
plt.xlabel('Number of Trees')
plt.ylabel('Accuracy')
plt.title('Cross-GAN: StyleGAN2 (SFHQ) → StyleGAN1 (Flickr)')
plt.legend()
plt.grid(True)
...
with open(os.path.join(folder_path, "results.txt"), "w") as f:
    ...
    f.write(f"Train Dataset: sfhq-part3 (StyleGAN2)\n")
    f.write(f"Test Dataset: flickr-gan-dataset (StyleGAN1)\n")
